...
const Btn = ({v="g",children,sx,...p}:any) => {
  const vs:any={g:{bg:"#008833",fg:"#011208",hv:"#00aa44"},b:{bg:"#082244",fg:"#3388ff",hv:"#0c2a54",bd:"1px solid #143a7a"},gh:{bg:"transparent",fg:"#3d5060",hv:"#0b1825",bd:"1px solid #192a38"},r:{bg:"#180505",fg:"#ff5555",hv:"#200808",bd:"1px solid #300a0a"},or:{bg:"#140800",fg:"#ff9900",hv:"#1c1000",bd:"1px solid #301800"},cy:{bg:"#011518",fg:"#00d4ff",hv:"#021c20",bd:"1px solid #003a44"},pu:{bg:"#0e040f",fg:"#cc44ff",hv:"#14061a",bd:"1px solid #2a0a30"}}[v]||{};
  return <button {...p} style={{display:"inline-flex",alignItems:"center",gap:6,padding:"9px 16px",borderRadius:7,cursor:"pointer",fontFamily:"inherit",fontSize:11,fontWeight:700,letterSpacing:.8,textTransform:"uppercase",border:vs.bd||"none",background:vs.bg,color:vs.fg,transition:"all .15s",...sx}} onMouseEnter={(e:any)=>{e.currentTarget.style.background=vs.hv;e.currentTarget.style.transform="translateY(-1px)"}} onMouseLeave={(e:any)=>{e.currentTarget.style.background=vs.bg;e.currentTarget.style.transform=""}}>{children}</button>;
};
...
const Spinner = ({msg="Cargando..."}:{msg?:string}) => (
  <div style={{display:"flex",alignItems:"center",justifyContent:"center",height:"100%",flexDirection:"column",gap:14,color:"#2a3d50"}}>
    <svg width={32} height={32} viewBox="0 0 24 24" fill="none" stroke="#00d4ff" strokeWidth={2} strokeLinecap="round" style={{animation:"spin 1s linear infinite"}}>
...
export default function App() {
  ...
  useEffect(()=>{
    if(!session) return;
    loadAll();
    const ch = sb.channel("gp_changes")
  ...
  const localeNames = locales.length > 0 ? locales.map((l:any)=>l.name) : ["Centro","Norte","Sur","Este","Oeste","Microcentro"];

  const getStk = (productId:number, localName:string) => {
    const s = stock.find(s=>s.productId===productId && s.localName===localName);
    return s ? s.stk : 0;
  };

  const prodsWithLocalStk = prods.map(p => ({
    ...p,
    stk: isAdmin ? p.stk : getStk(p.id, session?.local||"")
  }));
    ...
    <div style={{fontFamily:"'Outfit','Trebuchet MS',sans-serif",background:"#030810",minHeight:"100vh",color:"#bdd0e0"}}>
      ...
      <div style={{display:"flex",height:"100vh",overflow:"hidden"}}>
        ...
        <main style={{flex:1,overflow:"auto",padding:22}}>
          {loading ? <Spinner msg="Conectando..."/> : <>
            {view==="dash"     && <Dashboard prods={prodsWithLocalStk} stock={stock} clients={clients} sales={sales} users={users} session={session} isAdmin={isAdmin} setView={setView}/>}
            {view==="sale"     && <NewSale prods={prodsWithLocalStk} clients={clients} notify={notify} session={session} stock={stock} onSaved={loadAll}/>}
            {view==="history"  && <History sales={sales} clients={clients} users={users}/>}
            {view==="clients"  && <Clients clients={clients} sales={sales} notify={notify} isAdmin={isAdmin} onSaved={loadAll}/>}
            {view==="caja"     && <CashClose sales={sales} caja={caja} notify={notify} session={session} onSaved={loadAll}/>}
            {isAdmin&&view==="prods"    && <Products prods={prods} notify={notify} onSaved={loadAll}/>}
            {isAdmin&&view==="stockmgt" && <StockMgt prods={prods} stock={stock} notify={notify} onSaved={loadAll} localeNames={localeNames}/>}
            {isAdmin&&view==="localmgt" && <LocalMgt locales={locales} notify={notify} onSaved={loadAll}/>}
            {isAdmin&&view==="usermgt"  && <UserMgmt users={users} notify={notify} session={session} onSaved={loadAll} localeNames={localeNames}/>}
            {isAdmin&&view==="perfil"   && <AdminProfile session={session} setSession={setSession} notify={notify} onSaved={loadAll}/>}
          </>}
        </main>
      </div>
    </div>
          ...
          {[...neg,...bajo].length===0
            ...
            : [...neg,...bajo].map((p:any)=>{const[,,,em]=CAT_STYLE[p.cat]||["","","#fff",""];return(
              <div key={p.id} style={{padding:"8px 15px",borderBottom:"1px solid #192a3810",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                ...
                <div style={{textAlign:"right"}}><div style={{fontWeight:800,fontSize:13,color:p.stk<0?"#ff4444":"#ff9900"}}>{p.unit==="kg"?fmtWeight(p.stk):`${p.stk} u`}{p.stk<0?" ⚠":""}</div><div style={{fontSize:8,color:"#2a3d50"}}>mín:{p.min}</div></div>
              </div>
            );})}
        </Card>
      </div>
...
function NewSale({prods,clients,notify,session,stock,onSaved}:any) {
  ...
  const addToCart=(prod:any,type:string,inputValue:any)=>{
    ...
    if(type==="granel"&&prod.unit==="kg"){qty=val/prod.pricePerKg;sub_val=val;unitDisplay=fmtWeight(qty);}
    else if(type==="bulto"&&prod.unit==="kg"){qty=Math.round(val)*prod.bulkWeight;sub_val=Math.round(val)*prod.bulkPrice;unitDisplay=`${Math.round(val)} bulto${Math.round(val)>1?"s":""} (${fmtWeight(qty)})`;}
    else{qty=Math.round(val);sub_val=qty*(prod.unitPrice||0);unitDisplay=`${qty} u`;}
    const key=`${prod.id}-${type}`;
    setCart((prev:any[])=>{const ex=prev.find(i=>i.key===key);if(ex)return prev.map(i=>i.key===key?{...i,qty:i.qty+qty,sub:i.sub+sub_val,unitDisplay:type==="granel"&&prod.unit==="kg"?fmtWeight(i.qty+qty):unitDisplay}:i);return[...prev,{key,pid:prod.id,name:prod.name,unit:prod.unit,type,qty,sub:sub_val,unitDisplay}];});
  };
  ...
  const confirm=async()=>{
    ...
    try {
      ...
      await sb.from("gp_sales").insert([{id:saleId,date,cid:parseInt(cid),items:cart,sub,disc,total,pay,pts_e:ptsE,pts_s:ptsUs,uid:session.id,local_name:session.local||""}]);
      for(const it of cart){
        ...
        if(stkRow){
          await sb.from("gp_stock").update({stk:stkRow.stk-delta,updated_at:new Date().toISOString()}).eq("id",stkRow.id);
        } else {
  ...
  return (
    <div style={{background:"#06111e",border:`1px solid ${p.stk<0?"#330000":"#192a38"}`,borderRadius:10,padding:12,display:"flex",flexDirection:"column",gap:8,position:"relative"}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
        ...
        <div style={{fontSize:11,fontWeight:700,color:p.stk<0?"#ff4444":p.stk<=p.min?"#ff9900":"#2a3d50"}}>{isKg?fmtWeight(p.stk):`${p.stk} u`}</div>
      </div>
      {isKg&&<>
        <div style={{background:"#020e06",border:"1px solid #00882220",borderRadius:7,padding:"8px 10px"}}>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:4,alignItems:"center"}}><Chip t="granel"/><span style={{color:"#00cc55",fontWeight:700,fontSize:10}}>${p.pricePerKg.toFixed(2)}/kg</span></div>
          <div style={{background:"#060f1a",borderRadius:5,padding:"4px 8px",marginBottom:5,fontSize:10,color:"#00cc55",fontWeight:700}}>${granelMonto} = {fmtWeight(kgPorMonto)}</div>
          <div style={{display:"flex",gap:5,alignItems:"center"}}><Inp type="number" min="1" step="10" value={granelMonto} onChange={(e:any)=>setGranelMonto(parseFloat(e.target.value)||0)} sx={{width:70,fontSize:12}}/><span style={{fontSize:9,color:"#2a3d50"}}>$</span><Btn v="g" sx={{flex:1,justifyContent:"center",fontSize:9,padding:"4px 6px"}} onClick={()=>onAdd(p,"granel",granelMonto)}>+ Ag.</Btn></div>
        </div>
        {p.bulkWeight>0&&<div style={{background:"#02060e",border:"1px solid #2266ee20",borderRadius:7,padding:"8px 10px"}}>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:4,alignItems:"center"}}><Chip t="bulto"/><span style={{color:"#3388ff",fontWeight:700,fontSize:10}}>${p.bulkPrice} · {fmtWeight(p.bulkWeight)}</span></div>
...
function StockMgt({prods,stock,notify,onSaved,localeNames}:any) {
  ...
  const getStk=(pid:number)=>{
    const s=stock.find((s:any)=>s.productId===pid&&s.localName===localF);
    return s?s.stk:0;
  };
  ...
  const saveStk=async(prod:any)=>{
    ...
    try{
      ...
      if(existing){
        await sb.from("gp_stock").update({stk:newVal,updated_at:new Date().toISOString()}).eq("id",existing.id);
      } else {
        await sb.from("gp_stock").insert([{product_id:prod.id,local_name:localF,stk:newVal}]);
      }
    ...
    <div className="fade">
      ...
      <div style={{display:"flex",gap:8,marginBottom:12,flexWrap:"wrap"}}>
        {localeNames.map((l:string)=>(
          <button key={l} onClick={()=>setLocalF(l)} style={{background:localF===l?"#051626":"transparent",border:`1px solid ${localF===l?"#00d4ff":"#192a38"}`,color:localF===l?"#00d4ff":"#2a3d50",borderRadius:7,padding:"7px 14px",cursor:"pointer",fontFamily:"inherit",fontSize:11,fontWeight:700,transition:"all .15s"}}>
            📍 {l}
          </button>
        ))}
      </div>
      ...
      <Card sx={{overflow:"hidden"}}>
        <div style={{padding:"11px 16px",borderBottom:"1px solid #192a38",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <span style={{fontSize:8,fontWeight:700,letterSpacing:2.5,color:"#2a3d50",textTransform:"uppercase"}}>Stock · Local {localF}</span>
            ...
            {filtered.map((p:any)=>{
              ...
              return(
                <tr key={p.id}>
                  ...
                  <td><span style={{fontWeight:800,color:stk<0?"#ff4444":stk<=p.min?"#ff9900":"#00cc55"}}>{p.unit==="kg"?fmtWeight(stk):`${stk} u`}</span></td>
                  <td>
                    <Inp type="number" step={p.unit==="kg"?".5":"1"} placeholder={String(stk)} value={vals[p.id]??String(stk)} onChange={(e:any)=>setVals(v=>({...v,[p.id]:e.target.value}))} sx={{width:100,fontSize:12,background:edited?"#021408":"#060f1a",borderColor:edited?"#00cc55":"#192a38"}}/>
                  </td>
                  <td>
                    <Btn v="g" sx={{padding:"4px 10px",fontSize:9}} onClick={()=>saveStk(p)} disabled={saving===p.id}>
                      {saving===p.id?<Ic n="spin" s={11}/>:<Ic n="ok" s={11}/>}{saving===p.id?"...":"Guardar"}
                    </Btn>
                  </td>
                </tr>
              );
...
function LocalMgt({locales,notify,onSaved}:any) {
  ...
  const save=async()=>{
    ...
    try{
      ...
      notify(form.id?"Local actualizado":"Local creado");await onSaved();setModal(false);
    }catch(e){notify("Error","err");}
    setSaving(false);
  };

  const del=async(id:any)=>{
    await sb.from("gp_locales").delete().eq("id",id);
    notify("Local eliminado");setConfirmDel(null);await onSaved();
  };
  ...
  return(
    <div className="fade">
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
        <div>
          <h1 style={{fontSize:18,fontWeight:800,margin:0}}>Locales</h1>
          <p style={{color:"#2a3d50",fontSize:9,margin:"3px 0 0",letterSpacing:2.5}}>NOMBRES DE PUNTOS DE VENTA</p>
        </div>
        <Btn v="g" onClick={openNew}><Ic n="plus" s={13}/>Nuevo Local</Btn>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(220px,1fr))",gap:12}}>
        {locales.map((l:any)=>(
          <Card key={l.id} sx={{padding:16,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              <div style={{fontSize:22}}>📍</div>
              <div style={{fontSize:14,fontWeight:800,color:"#bdd0e0"}}>{l.name}</div>
            </div>
            <div style={{display:"flex",gap:5}}>
              <Btn v="gh" sx={{padding:"3px 6px",fontSize:9}} onClick={()=>openEdit(l)}><Ic n="edit" s={11}/></Btn>
              <Btn v="r" sx={{padding:"3px 6px",fontSize:9}} onClick={()=>setConfirmDel(l)}><Ic n="del" s={11}/></Btn>
            </div>
          </Card>
        ))}
      ...
      {modal&&form&&(
        <Modal close={()=>setModal(false)} w={360}>
          <div style={{padding:22}}>
            <h2 style={{margin:"0 0 16px",fontSize:15,fontWeight:800}}>{form.id?"Editar":"Nuevo"} Local</h2>
            <Lbl t="Nombre del Local"/>
            <Inp value={form.name} onChange={(e:any)=>setForm((f:any)=>({...f,name:e.target.value}))} placeholder="ej: Sucursal Centro"/>
      ...
      {confirmDel&&(
        <Modal close={()=>setConfirmDel(null)} w={360}>
          <div style={{padding:24,textAlign:"center"}}>
            ...
            <h2 style={{margin:"0 0 8px",fontSize:16,fontWeight:800}}>¿Eliminar local?</h2>
            <p style={{color:"#6a8090",fontSize:13,marginBottom:20}}><strong style={{color:"#a0bcd0"}}>{confirmDel.name}</strong></p>
            <div style={{display:"flex",gap:10,justifyContent:"center"}}>
              <Btn v="gh" onClick={()=>setConfirmDel(null)}>Cancelar</Btn>
              <Btn v="r" onClick={()=>del(confirmDel.id)}><Ic n="del" s={13}/>Eliminar</Btn>
            </div>
          </div>
        </Modal>
      )}
...
function History({sales,clients,users}:any) {
  const[q,setQ]=useState("");const[pf,setPf]=useState("todos");const[det,setDet]=useState<any>(null);
...
function Clients({clients,sales,notify,isAdmin,onSaved}:any) {
  ...
  const save=async()=>{
    ...
    try{
      ...
      notify(form.id?"Actualizado":"Cliente creado");await onSaved();setModal(false);
    }catch(e){notify("Error","err");}
    setSaving(false);
  };
  const del=async(id:any)=>{ await sb.from("gp_clients").delete().eq("id",id); notify("Eliminado");setConfirmDel(null);await onSaved(); };
  const doRedeem=async()=>{
    ...
    notify(`${pts} pts canjeados`);setRdm(null);setRpts(0);await onSaved();
  };
...
function CashClose({sales,caja,notify,session,onSaved}:any) {
  ...
  const doClose=async()=>{
    ...
    try{
      ...
      notify(`Caja cerrada. $${totalAll.toFixed(2)}`);setClosing(false);setOpenAmt("");setNotes("");await onSaved();
    }catch(e){notify("Error","err");}
    setSaving(false);
  };
  return(
...
function Products({prods,notify,onSaved}:any) {
  ...
  const openNew=()=>{setForm({name:"",code:"",cat:"Perro",unit:"kg",pricePerKg:0,bulkWeight:25,bulkPrice:0,unitPrice:0,stk:0,min:0});setModal(true);};
  const openEdit=(p:any)=>{setForm({...p});setModal(true);};
  const save=async()=>{
    if(!form.name.trim()){notify("Nombre requerido","err");return;}
    setSaving(true);
    const payload={code:form.code||"",name:form.name,cat:form.cat,unit:form.unit,price_per_kg:form.pricePerKg||0,bulk_weight:form.bulkWeight||0,bulk_price:form.bulkPrice||0,unit_price:form.unitPrice||0,stk:form.stk||0,min_stk:form.min||0,updated_at:new Date().toISOString()};
    try{
      if(form.id) await sb.from("gp_products").update(payload).eq("id",form.id);
        ...
        if(newProd){
          ...
          for(const loc of locNames){
            await sb.from("gp_stock").insert([{product_id:newProd.id,local_name:loc,stk:0}]);
          }
        }
      }
      notify(form.id?"Actualizado":"Creado");await onSaved();setModal(false);
    }catch(e){notify("Error","err");}
    setSaving(false);
  };
  const del=async(id:any)=>{ await sb.from("gp_products").delete().eq("id",id); notify("Eliminado");setConfirmDel(null);await onSaved(); };
  const isKg=form?.unit==="kg";
  const vis=prods.filter((p:any)=>p.name.toLowerCase().includes(q.toLowerCase())&&(catF==="Todas"||p.cat===catF));
  return(
    <div className="fade">
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}><div><h1 style={{fontSize:18,fontWeight:800,margin:0}}>Productos</h1><p style={{color:"#2a3d50",fontSize:9,margin:"3px 0 0",letterSpacing:2.5}}>{prods.length} REGISTROS · precios y datos globales</p></div><Btn v="g" onClick={openNew}><Ic n="plus" s={13}/>Nuevo</Btn></div>
...
function UserMgmt({users,notify,session,onSaved,localeNames}:any) {
  ...
  const toggle=async(u:any)=>{ if(u.id===session.id){notify("No podés desactivarte","err");return;} await sb.from("gp_users").update({active:!u.active}).eq("id",u.id); await onSaved(); };
  const del=async(id:any)=>{ if(id===session.id){notify("No podés eliminarte","err");return;} await sb.from("gp_users").delete().eq("id",id); notify("Eliminado");await onSaved(); };
...
function AdminProfile({session,setSession,notify,onSaved}:any) {
  ...
  const saveProfile=async()=>{
    ...
    try{
      ...
      notify("Perfil actualizado ✓");await onSaved();
    }catch(e){notify("Error","err");}
    setSaving(false);
  };
