...
function NewSale({prods,clients,notify,session,stock,loadAll}:any) {
  ...
  const confirm=async()=>{
    ...
    try{
      ...
      // Mostramos recibo ANTES de recargar
...
function History({sales,clients,users,isAdmin,notify,loadAll}:any) {
  ...
  const delSale=async(id:any)=>{
    await sb.from("gp_sales").delete().eq("id",id);
    notify("Venta eliminada");setConfirmDel(null);loadAll();
  };
  return(
    <div className="fade">
...
function Reportes({sales,users,localeNames}:any) {
  ...
  const byLocal=localeNames.map((l:string)=>{
    const ls=sales.filter((s:any)=>s.localName===l);
    return{name:l,count:ls.length,total:ls.reduce((a:number,b:any)=>a+b.total,0)};
  });
  const byUser=users.map((u:any)=>{
    const us=sales.filter((s:any)=>s.uid===u.id);
    return{name:u.name,local:u.local,role:u.role,count:us.length,total:us.reduce((a:number,b:any)=>a+b.total,0)};
  }).filter((u:any)=>u.count>0).sort((a:any,b:any)=>b.total-a.total);
  ...
  return(
    <div className="fade">
      ...
      {tab==="local"&&(
        <Card sx={{overflow:"hidden"}}>
          <div style={{padding:"11px 16px",borderBottom:"1px solid #192a38"}}><span style={{fontSize:8,fontWeight:700,letterSpacing:2.5,color:"#2a3d50",textTransform:"uppercase"}}>Ventas por Local</span></div>
          <table><thead><tr><th>Local</th><th>Ventas</th><th>Total</th><th>% del Total</th></tr></thead>
            <tbody>{byLocal.sort((a:any,b:any)=>b.total-a.total).map((l:any)=>(
              <tr key={l.name}>
                <td style={{fontWeight:700,color:"#a0bcd0"}}>📍 {l.name}</td>
                <td>{l.count}</td>
                <td style={{fontWeight:800,color:"#00cc55",fontSize:13}}>${l.total.toFixed(2)}</td>
                <td>
                  <div style={{display:"flex",alignItems:"center",gap:8}}>
                    <div style={{flex:1,height:6,background:"#192a38",borderRadius:3,overflow:"hidden"}}>
                      <div style={{height:"100%",background:"#00cc55",width:`${totalGeneral>0?(l.total/totalGeneral*100):0}%`,borderRadius:3}}/>
                    </div>
                    <span style={{fontSize:10,color:"#2a3d50",minWidth:36}}>{totalGeneral>0?(l.total/totalGeneral*100).toFixed(1):0}%</span>
                  </div>
                </td>
              </tr>
            ))}</tbody>
      ...
      {tab==="user"&&(
        <Card sx={{overflow:"hidden"}}>
          <div style={{padding:"11px 16px",borderBottom:"1px solid #192a38"}}><span style={{fontSize:8,fontWeight:700,letterSpacing:2.5,color:"#2a3d50",textTransform:"uppercase"}}>Ventas por Usuario</span></div>
          <table><thead><tr><th>Usuario</th><th>Rol</th><th>Local</th><th>Ventas</th><th>Total</th><th>% del Total</th></tr></thead>
            <tbody>{byUser.map((u:any,i:number)=>(
              <tr key={i}>
                <td style={{fontWeight:700,color:"#a0bcd0"}}>{u.name}</td>
                <td><Chip t={u.role}/></td>
                <td style={{color:"#00d4ff"}}>{u.local||"—"}</td>
                <td>{u.count}</td>
                <td style={{fontWeight:800,color:"#00cc55",fontSize:13}}>${u.total.toFixed(2)}</td>
                <td>
                  <div style={{display:"flex",alignItems:"center",gap:8}}>
                    <div style={{flex:1,height:6,background:"#192a38",borderRadius:3,overflow:"hidden"}}>
                      <div style={{height:"100%",background:"#3388ff",width:`${totalGeneral>0?(u.total/totalGeneral*100):0}%`,borderRadius:3}}/>
                    </div>
                    <span style={{fontSize:10,color:"#2a3d50",minWidth:36}}>{totalGeneral>0?(u.total/totalGeneral*100).toFixed(1):0}%</span>
                  </div>
                </td>
              </tr>
            ))}</tbody>
  ...
  return(
    <div className="fade">
      ...
      <div style={{display:"flex",gap:8,marginBottom:12,flexWrap:"wrap"}}>
        {localeNames.map((l:string)=>(<button key={l} onClick={()=>setLocalF(l)} style={{background:localF===l?"#051626":"transparent",border:`1px solid ${localF===l?"#00d4ff":"#192a38"}`,color:localF===l?"#00d4ff":"#2a3d50",borderRadius:7,padding:"7px 14px",cursor:"pointer",fontFamily:"inherit",fontSize:11,fontWeight:700,transition:"all .15s"}}>📍 {l}</button>))}
      </div>
